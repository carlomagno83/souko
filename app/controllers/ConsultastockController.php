...
//kardex
	...
	public function pantallaexcelkardexdescanso()
	{

		$sql = "SELECT fechadocumento 
					FROM documentos
					INNER JOIN movimientos ON movimientos.documento_id=documentos.id
					GROUP BY fechadocumento
					ORDER BY fechadocumento DESC
					LIMIT 1";
		//ya tenemos el ultimo dia con movimientos
		
		$encuentra= DB::select($sql);	
		//dd($encuentra[0]->fechadocumento);		
		return View::make('consultastock.pantallakardexdescanso')->withErrors(['Kardex se generará con los movimientos encontrados el día '. $encuentra[0]->fechadocumento]);
	...
	public function descargaexcelkardexdescanso()
	{

		//hay que buscar si hay movimiento
		$diasretraso = 1;
		for ($diasretraso = 1; $diasretraso <= 10; $diasretraso++)
		{
			$sql = "SELECT movimientos.documento_id FROM documentos
						INNER JOIN movimientos ON movimientos.documento_id=documentos.id
						WHERE fechadocumento=DATE_SUB(CONCAT(CURDATE(), ' 00:00:00'), 
						INTERVAL " .$diasretraso. " DAY)";

			$encuentra= DB::select($sql);
			if (count($encuentra) > 0) break; 

		}	
		//ya tenemos los dias de retraso en la variable $diasretraso
//dd($diasretraso);
		...
		$sql = "SELECT fechadocumento 
					...
					LIMIT 1";
		$encuentra= DB::select($sql);	
		$fec = $encuentra[0]->fechadocumento;
		//dd($encuentra[0]->fechadocumento);
			...
			$sql = "SELECT fechadocumento, desusuario, codlocal3, movimientos.mercaderia_id, codproducto31, 
						...
						INNER JOIN users ON mercaderias.usuario_id=users.id
						INNER JOIN locals ON mercaderias.local_id=locals.id
						WHERE fechadocumento = '$fec' AND movimientos.tipomovimiento_id=3
						ORDER BY desusuario, codlocal3";
...
//kardex
			...
			$sheet->setHeight($i, 15);
		  	...
		  	$sheet->cells('A1:B1', function($cells) { $cells->setBackground('#FFFFFF'); });
		        $sheet->loadView('consultastock.kardexxcdescanso')->with('mercaderias', $mercaderias);
