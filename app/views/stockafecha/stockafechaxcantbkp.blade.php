...
@while($key<=count($mercaderias)-1 )
    ...
    @if($gen1 < $gen2 and $fin==0)
        ...
        <td> {{$mercaderias[$key]->codmarca3}}-{{$mercaderias[$key]->codtipo8}}-{{$mercaderias[$key]->codrango6}} </td> 
        @for ($i = 1; $i <= $cantidad_locales; $i++) 
            <td>{{$mercaderias[$key]->$expresion[$i-1]}}</td>
        @endfor
        <?php $key++ ; ?>
    @endif
    @if($gen1 == $gen2 and $fin==0)
        <td> {{$mercaderias[$key]->codmarca3}}-{{$mercaderias[$key]->codtipo8}}-{{$mercaderias[$key]->codrango6}} </td> 
        @for ($i = 1; $i <= $cantidad_locales; $i++) 
            <td>{{$mercaderias[$key]->$expresion[$i-1] + $movimientos[$key2]->$expresion[$i-1]}}</td>
        @endfor
        <?php 
            $key++ ;
